...
class AnchorHeadPRITemplate(nn.Module):
    def __init__(self, model_cfg, num_class, class_names, grid_size, point_cloud_range, predict_boxes_when_training):
        ...
        self.conv_cls_information = nn.Conv1d(1, 4, kernel_size=1)
        self.conv_box_information = nn.Conv1d(7, 28, kernel_size=1)
        self.conv_dir_information = nn.Conv1d(1, 4, kernel_size=1)

        # voxelrcnn
        self.conv_importance = nn.Sequential(
            nn.Conv1d(296, 64, kernel_size=1),
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.Dropout(0.2),
    ...
    def get_importance(self, cls_loss_src, loc_loss_src, dir_loss):
        cls_preds = self.forward_ret_dict['cls_preds']
        # print("cls_loss_src:", cls_loss_src.shape)
        # print("loc_loss_src:", loc_loss_src.shape)
        spatial_features = self.forward_ret_dict['st_features']
        # print("spatial_features:", spatial_features.shape)
        spatial_features = torch.cat((spatial_features, spatial_features), dim=1).permute(0, 2, 1).contiguous().detach()
        batch_size = int(cls_preds.shape[0])
        # print(self.forward_ret_dict['pts_ratio_weight'].view(batch_size, 1, -1).shape)

        pr_information = self.forward_ret_dict['pts_ratio_weight'].view(batch_size, 1, -1).detach() # [2, 1, 43040]
        cls_loss_src = cls_loss_src.permute(0, 2, 1).contiguous().detach()
        loc_loss_src = loc_loss_src.permute(0, 2, 1).contiguous().detach()
        dir_loss = dir_loss.view(batch_size, -1, 1).permute(0, 2, 1).contiguous().detach()
